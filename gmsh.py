...
class Mesh:
    ...
    Elmts : dict
        ...
        (rank 1 array of physical ids, rank 2 array of element to vertex ids (Nel x ppe))
        each array in the tuple is of length nElmts
    Phys : dict
        keys and names
        ...
        while line:
            ...
            if line.find('$Nodes')==0:
                ...
                for i in range(0,self.npts):
                    line=fid.readline()
                    data = line.split()
                    id = int(data[0])-1 # fix gmsh 1-based indexing
                    if i!=id:
                        raise ValueError, 'problem with vertex ids'
                    self.Verts[id,:] = map(float,data[1:])
                line=fid.readline()
            ...
            if line.find('$Elements')==0:
                ...
                for i in range(0,self.nel):
                    ...
                    if ntags > 0:                   # set physical id
                        physid = int(data[k])
                        if not self.Phys.has_key(physid):
                    ...
                    if (not self.Elmts.has_key(etype)) or (len(self.Elmts[etype])==0):
                        # initialize
    ...
    def write_neu(self,fname=None):
        ...
        for i in range(0,nel):
            ...
            fid.write(dstr+'\n')
        fid.write('ENDOFSECTION\n')
        #    Write all the boundary condition blocks
        #    IMPORTANT, it is assumed that the the mesh_condition_name
        #    stores the BC type and the first number to follow the BC in the .neu file.
        #    For circular boundaries, mesh_condition_name[i] = "circ  _radius-length_"
        #    For standard dirichlet boundaries, mesh_condition_name[i] = "diri 1".  
        #    examples
        #    Wall 1
        #    Inflow 1
        #    Outflow 1
    ...
    def _elm_types(self):
        ...
        elm_type[26] = 4    # 4-node third order edge 
                            # (2 nodes associated with the vertices, 2 internal to the edge) 
        elm_type[27] = 5    # 5-node fourth order edge 
                            # (2 nodes associated with the vertices, 3 internal to the edge) 
        elm_type[28] = 6    # 6-node fifth order edge 
                            # (2 nodes associated with the vertices, 4 internal to the edge) 
    ...
    def refine2dtri(self, marked_elements=None):
        ...
        nsplit = len(numpy.where(marked_edges==True)[0])
        ...
        #   /       \  /  |
        # n0 --------n3-- n1
        id = numpy.ones((Nel,),dtype=bool)
        id[edges3]=False
        id[edges1]=False
        id2 = numpy.where(id==True)[0]
    ...
    def smooth2dtri(self, maxit=10, tol=0.01):
        ...
        #S = sparse(mesh.tri(:,[1,1,2,2,3,3]),mesh.tri(:,[2,3,1,3,1,2]),1,mesh.n,mesh.n);
        S = coo_matrix((data,(edge0,edge1)),shape=(self.Verts.shape[0],self.Verts.shape[0])).tocsr().tocoo()
        S0 = S.copy()
        S.data = 0*S.data + 1
