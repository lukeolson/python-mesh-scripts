...
def regular_triangle_mesh(nx, ny):
    ...
    Vert1 = numpy.tile(numpy.arange(0, nx - 1), ny - 1) + numpy.repeat(
        numpy.arange(0, nx * (ny - 1), nx), nx - 1
    )
    Vert3 = numpy.tile(numpy.arange(0, nx - 1), ny - 1) + numpy.repeat(
        numpy.arange(0, nx * (ny - 1), nx), nx - 1
    ) + nx
    Vert2 = Vert3 + 1
    Vert4 = Vert1 + 1

    Verttmp = numpy.meshgrid(
        numpy.arange(0, nx, dtype="float"), numpy.arange(0, ny, dtype="float")
    )
...
if __name__ == "__main__":
    ...
    if meshnum == 3:
        V, E = regular_triangle_mesh(10, 8)

    import matplotlib.pyplot as plt
    ax = trimesh(V, E, labels=True)
    plt.show()
